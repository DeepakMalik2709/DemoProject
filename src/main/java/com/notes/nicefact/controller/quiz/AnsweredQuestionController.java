...
public class AnsweredQuestionController extends CommonController {
	...
	@Path("/query")	
	@Consumes(MediaType.APPLICATION_JSON)
	public void queryRecord(AnsweredQuestionTO answeredQuestionTO,@Context HttpServletResponse response,@Context HttpServletRequest request) {
		logger.info("get all question start , postId : ");
		...
		try {
			AnsweredQuestionService ansQuestionService = new AnsweredQuestionService(em);
			AppUser user = (AppUser) request.getSession().getAttribute(Constants.SESSION_KEY_lOGIN_USER);	
		//	AnsweredQuestionTO answeredQuestionTO = new AnsweredQuestionTO(Long.parseLong(quizId)); 
			List<AnsweredQuestionTO> ansQuesTOList =ansQuestionService.queryRecord(answeredQuestionTO,user);
			
			json.put(Constants.CODE, Constants.RESPONSE_OK);
			json.put(Constants.DATA_ITEMS, ansQuesTOList);
